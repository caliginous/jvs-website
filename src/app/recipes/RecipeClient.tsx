...
export default function RecipeClient({ initialRecipes }: RecipeClientProps) {
  ...
  // Client-side fallback: if no recipes provided from server, fetch from API and normalize
  useEffect(() => {
    const needsFallback = !initialRecipes || initialRecipes.length === 0;
    ...
    (async () => {
      try {
        ...
        if (!res.ok) return;
        const sanityRecipes = await res.json();
        if (aborted || !Array.isArray(sanityRecipes)) return;
        const mapped: Recipe[] = sanityRecipes.map((r: any) => {
          const slug = r?.slug?.current || '';
          const featured = r?.externalImageUrl || r?.featuredImage?.asset?.url || '';
          const ingredients = Array.isArray(r?.ingredients)
            ? r.ingredients.map((i: any) => (typeof i === 'string' ? i : i?.item || '')).filter(Boolean)
            : [];
          const instructions = Array.isArray(r?.instructions)
            ? r.instructions.map((i: any) => (typeof i === 'string' ? i : i?.text || '')).filter(Boolean)
            : [];
          return {
            id: r?._id || slug || String(Math.random()),
            title: r?.title || 'Untitled',
            slug,
            date: r?.publishedAt || r?._createdAt || new Date().toISOString(),
            excerpt: '',
            ...
            preparationTime: r?.preparationTime ? String(r.preparationTime) : undefined,
            cookTime: r?.cookTime ? String(r.cookTime) : undefined,
            difficulty: r?.difficulty || undefined,
            categories: Array.isArray(r?.categories) ? r.categories : [],
            tags: Array.isArray(r?.tags) ? r.tags : [],
            source: 'sanity',
            createdAt: r?.publishedAt || r?._createdAt || new Date().toISOString(),
            updatedAt: r?._updatedAt || r?.publishedAt || r?._createdAt || new Date().toISOString(),
          } as Recipe;
        });
        // sort newest-first
  ...
  const enhancedRecipes: EnhancedRecipe[] = recipes
  ...
  // Sort newest-first by date (server already sorts, but enforce in client too)
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Discover delicious vegan recipes inspired by Jewish culinary traditions
          </p>
        </div>
