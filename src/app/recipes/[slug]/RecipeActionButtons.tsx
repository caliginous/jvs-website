...
export default function RecipeActionButtons({ recipe }: RecipeActionButtonsProps) {
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        ...
        text: `Check out this recipe: ${decodeHtmlEntities(recipe.title)}`,
  ...
  const handleBookmark = () => {
    ...
    const isBookmarked = bookmarks.includes(recipe.slug);
    
    if (isBookmarked) {
      const newBookmarks = bookmarks.filter((slug: string) => slug !== recipe.slug);
      localStorage.setItem('recipeBookmarks', JSON.stringify(newBookmarks));
      alert('Recipe removed from bookmarks!');
    } else {
      bookmarks.push(recipe.slug);
