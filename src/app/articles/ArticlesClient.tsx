...
export default function ArticlesClient({ initialArticles }: ArticlesClientProps) {
  ...
  const enhancedArticles: EnhancedArticle[] = initialArticles
    ...
    .filter(article => {
      // Filter out articles with excluded categories
      if (article.categories?.nodes) {
        ...
        return !article.categories.nodes.some(category => 
  ...
  // Extract unique categories from all articles (excluding excluded categories)
  ...
  // Filter articles by search term and selected categories
  const filteredArticles = enhancedArticles.filter((article: EnhancedArticle) => {
    const matchesSearch = article.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         (article.excerpt && article.excerpt.toLowerCase().includes(searchTerm.toLowerCase()));
    
    const matchesCategories = selectedCategories.length === 0 || 
      (article.categories?.nodes && selectedCategories.some(cat => 
        article.categories!.nodes.some(category => category.name === cat)
      ));
  ...
  const handleCategoryToggle = (category: string) => {
    ...
    setDisplayCount(50); // Reset to initial count when filters change
  };
