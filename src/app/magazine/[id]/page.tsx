...
export default function MagazineDetailPage() {
  ...
  const id = (params.id as string)
    .replace('jvs-mag-online-', 'jvs-mag-')
    .replace('magonline', 'mag-')
    .replace('online', '')
    .replace(/--+/g, '-')
    .replace(/-$/, '');
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-lg text-gray-600 mt-2">{formatDate(magazine.date)}</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="lg:col-span-2">
            ...
            <div className="bg-white rounded-lg shadow-md p-6 mb-8">
              ...
              <div className="text-center py-8">
                ...
                <p className="text-gray-600 mb-4">Download or view this issue</p>
                <a href={magazine.pdf_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors mr-3">
                  <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                  </svg>
                  Download PDF
                </a>
                <button onClick={()=>setShowInlinePdf(!showInlinePdf)} className="inline-flex items-center px-6 py-3 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                  {showInlinePdf ? 'Hide inline preview' : 'View inline'}
                </button>
              </div>

              {showInlinePdf && (
                <div className="mt-6 border rounded overflow-hidden">
                  <object data={magazine.pdf_url} type="application/pdf" width="100%" height="800px">
                    <iframe src={magazine.pdf_url} width="100%" height="800px" title="PDF Preview" />
                  </object>
                </div>
              )}
            </div>
