...
export default {
  async fetch(request, env, ctx) {
    ...
    // Handle events listing page
    if (url.pathname === '/events' || url.pathname === '/events/') {
      console.log('📅 [CUSTOM WORKER] Handling events listing page');
      return await handleEventsListingPage(request, env);
    }
    
    // Handle individual event pages
    if (url.pathname.startsWith('/events/') && !url.pathname.includes('/tickets') && !url.pathname.includes('/unified')) {
      console.log('📅 [CUSTOM WORKER] Handling individual event page');
      return await handleIndividualEventPage(request, env);
    }
    
    // Handle checkout page - serve dynamically
    if (url.pathname === '/checkout' || url.pathname === '/checkout/') {
      console.log('💳 [CUSTOM WORKER] Handling checkout page');
      return await handleCheckoutPage(request, env);
    }
    
    // Handle article pages - serve static files from assets
    if (url.pathname.startsWith('/articles/') && url.pathname !== '/articles/' && url.pathname !== '/articles') {
      console.log('📄 [CUSTOM WORKER] Handling article page');
      try {
        // Try to serve the static file from the assets
        const asset = await env.ASSETS.fetch(request);
        if (asset.status !== 404) {
          return asset;
        }
      } catch (error) {
        console.error('❌ [CUSTOM WORKER] Failed to serve article asset:', error);
      }
    }
    
    // For all other routes, serve static files from the assets
    console.log('📄 [CUSTOM WORKER] Serving static files from assets');
    ...
    try {
      ...
      console.error('❌ [CUSTOM WORKER] Failed to serve static asset:', error);
    }
...
async function handleGraphQLAPI(request, env) {
  ...
  try {
    ...
    // Staging uses production WordPress GraphQL since there's no staging WordPress site
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                ...
                <div class="col-span-1 md:col-span-2">
                    ...
                    <div class="flex space-x-4">
                        ...
                        <a href="https://www.facebook.com/JewishVegUK" class="text-neutral-400 hover:text-[#4FC3F7]">
                            <span class="sr-only">Facebook</span>
                            <svg class="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
                                <path fillRule="evenodd" d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" clipRule="evenodd" />
                            </svg>
                        </a>
                        <a href="https://www.instagram.com/jewishvegsoc/" class="text-neutral-400 hover:text-[#4FC3F7]">
...
<body class="bg-gray-50">
    ...
    <noscript>
        <iframe src="https://www.googletagmanager.com/ns.html?id=GTM-TJT5TZLX"
    ...
    <script>
        ...
        window.dataLayer.push({
            ...
            'page_title': '${event.name} - Unified Event | JVS'
        });
        ...
        window.dataLayer.push({
            ...
            'event_price': '${event.eventPrice || '0'}',
            'event_category': 'JVS Event'
        });
        ...
        function updateQuantity(index, change) {
            ...
            if (newQuantity !== oldQuantity) {
                window.dataLayer.push({
                    'event': 'ticket_quantity_change',
                    'event_id': '${event.id}',
                    'event_name': '${event.name}',
                    'ticket_type': ticketTypes[index].label,
                    'ticket_price': ticketTypes[index].price,
        ...
        function handlePurchase() {
            ...
            const totalPrice = quantities.reduce((sum, qty, index) => sum + (qty * parseFloat(ticketTypes[index].price || 0)), 0);
            const ticketTypesSelected = quantities.map((qty, index) => ({
                type: ticketTypes[index].label,
                quantity: qty,
            ...
            window.dataLayer.push({
                ...
                'ticket_types': ticketTypesSelected,
                'currency': 'GBP',
                'checkout_url': \`/checkout?eventId=${event.id}&quantity=\${totalTickets}\`
            });
...
// Events listing page handler
async function handleEventsListingPage(request, env) {
  try {
    console.log('📅 [EVENTS LISTING] Fetching events data');
    ...
    if (!graphqlResponse.ok) {
      console.error('❌ [EVENTS LISTING] GraphQL request failed');
    ...
    console.log(`✅ [EVENTS LISTING] Found ${events.length} events`);
    
    // Generate HTML for events listing page
    const html = generateEventsListingHTML(events);
...
// Individual event page handler
async function handleIndividualEventPage(request, env) {
  try {
    const url = new URL(request.url);
    const pathParts = url.pathname.split('/');
    const slug = pathParts[2]; // /events/[slug] -> slug is at index 2
    
    console.log(`📅 [INDIVIDUAL EVENT] Fetching event data for slug: ${slug}`);
    
    // Fetch event data from GraphQL
    const graphqlQuery = `
      query Events {
        eventProducts {
          id
    ...
    if (!graphqlResponse.ok) {
      console.error('❌ [INDIVIDUAL EVENT] GraphQL request failed');
    ...
    if (!event) {
      console.error(`❌ [INDIVIDUAL EVENT] Event not found: ${slug}`);
      return new Response('Event not found', { status: 404 });
    }
    
    console.log(`✅ [INDIVIDUAL EVENT] Found event: ${event.name}`);
    
    // Generate HTML for individual event page
    const html = generateIndividualEventHTML(event, slug);
    ...
    console.error('❌ [INDIVIDUAL EVENT] Error:', error);
    return new Response('Internal Server Error', { status: 500 });
  }
}
...
// Generate HTML for events listing page
function generateEventsListingHTML(events) {
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Events - Jewish Vegetarian Society</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <script>
        tailwind.config = {
            theme: {
...
<body class="bg-gray-50">
    ...
    <!-- Hero Section -->
    <section class="bg-[#E1F0D1] text-gray-900">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            <div class="text-center">
                <h1 class="text-4xl md:text-5xl font-bold mb-6">Events</h1>
                <p class="text-xl text-gray-700 max-w-3xl mx-auto">
                    Join us for inspiring events that promote Jewish values through veganism and sustainability.
                </p>
            </div>
        </div>
    </section>
    ...
    <section class="py-16">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center mb-8">
                <h2 class="text-3xl font-bold text-neutral-900">Upcoming Events</h2>
            </div>
                ...
                ${upcomingEvents.map(event => {
                    ...
                    <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 h-full flex flex-col">
                        <div class="p-6 flex flex-col h-full">
                            <div class="mb-3 min-h-[32px] flex items-start">
                                <span class="inline-block px-3 py-1 rounded-full text-sm font-medium bg-green-50 text-green-700 border border-green-200">
                                    🟢 Upcoming
                                </span>
                            </div>
                            
                            <div class="mb-4 min-h-[80px]">
                                <h3 class="text-xl font-semibold text-neutral-900 mb-2 line-clamp-2">
                                    <a href="/events/${event.id}/unified" class="hover:text-[#8BC34A] transition-colors">
                                        ${event.name}
                                    </a>
                                </h3>
                            ...
                            <div class="mt-auto">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center space-x-2">
                                        <span class="text-lg font-bold text-primary-400">${displayPrice}</span>
                                    </div>
                                    <div class="flex space-x-2">
                                        <a href="/events/${event.id}/unified" class="bg-neutral-100 hover:bg-neutral-200 text-neutral-700 px-3 py-2 rounded-lg text-sm font-medium transition-colors">
                                            Details
                                        </a>
                                        ${hasTickets ? `
                                        <a href="/events/${event.id}/unified" class="bg-primary-400 hover:bg-primary-500 text-white px-3 py-2 rounded-lg text-sm font-medium transition-colors">
                                            Buy Tickets
                                        </a>
                                        ` : ''}
                                    </div>
    ...
    <section class="py-16 bg-neutral-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center mb-8">
                <h2 class="text-3xl font-bold text-neutral-900">Past Events</h2>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ${pastEvents.map(event => {
                    ...
                    <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300 h-full flex flex-col">
                        <div class="p-6 flex flex-col h-full">
                            <div class="mb-3 min-h-[32px] flex items-start">
                                <span class="inline-block px-3 py-1 rounded-full text-sm font-medium bg-gray-50 text-gray-700 border border-gray-200">
                                    📅 Past Event
                                </span>
                            </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                ...
                <div>
                    <h3 class="text-lg font-semibold mb-4">Quick Links</h3>
                    <ul class="space-y-2">
                        <li><a href="/articles" class="text-neutral-300 hover:text-[#4FC3F7] transition-colors">Articles</a></li>
                        <li><a href="/events" class="text-neutral-300 hover:text-[#4FC3F7] transition-colors">Events</a></li>
                        <li><a href="/recipes" class="text-neutral-300 hover:text-[#4FC3F7] transition-colors">Recipes</a></li>
                        <li><a href="/about" class="text-neutral-300 hover:text-[#4FC3F7] transition-colors">About</a></li>
                    </ul>
                </div>
...
// Generate HTML for individual event page
function generateIndividualEventHTML(event, slug) {
  const eventDate = event.eventDate ? new Date(event.eventDate).toLocaleDateString('en-GB', {
    weekday: 'long',
    year: 'numeric',
    month: 'long',
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>${event.name} - Jewish Vegetarian Society</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <script>
        tailwind.config = {
            theme: {
...
<body class="bg-gray-50">
    ...
    <section class="text-white" style="background-color: #1f4d1a;">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            <div class="text-center">
                <h1 class="text-4xl md:text-5xl font-bold mb-4 text-white drop-shadow-lg">${event.name}</h1>
                ${eventDate !== 'TBD' ? `<p class="text-xl mb-4 text-white font-medium drop-shadow">${eventDate}</p>` : ''}
                ${isUpcoming ? '<span class="bg-green-100 text-green-800 px-4 py-2 rounded-full text-sm font-medium shadow-sm">Upcoming Event</span>' : ''}
            </div>
        </div>
    </section>
                ...
                <div class="p-8">
                    ...
                    <div class="grid grid-cols-1 gap-8 mb-8">
                        ...
                        <div class="flex items-start">
                            ...
                            <div>
                                <h3 class="font-semibold text-gray-900 mb-2">Date & Time</h3>
                                <p class="text-gray-600 text-lg">${eventDate}</p>
                            </div>
                        </div>
                        ` : ''}
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 pt-8 border-t border-gray-200">
                        ...
                        ${hasTickets && isUpcoming ? `
                        <a href="/events/${slug}/tickets" class="inline-flex items-center justify-center px-8 py-4 bg-primary-600 hover:bg-primary-700 text-white rounded-lg font-semibold transition-colors duration-200">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
async function handleCheckoutPage(request, env) {
  console.log('💳 [CUSTOM WORKER] Handling checkout page request');
  
  try {
    // Get the checkout page HTML from the static assets
    const checkoutRequest = new Request(new URL('/checkout/index.html', request.url), {
      method: 'GET',
      headers: request.headers
    });
    
    const response = await env.ASSETS.fetch(checkoutRequest);
    
    if (response.status === 404) {
      console.log('❌ [CUSTOM WORKER] Checkout page not found in static assets');
      return new Response('Checkout page not found', { status: 404 });
    }
    
    console.log('✅ [CUSTOM WORKER] Successfully served checkout page');
    return response;
    
  } catch (error) {
    console.error('❌ [CUSTOM WORKER] Error handling checkout page:', error);
    return new Response('Internal server error', { status: 500 });
  }
}